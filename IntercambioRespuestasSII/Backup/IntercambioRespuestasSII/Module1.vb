    ...
    Sub Main()
        ...
        If cantProc > 0 Then
            ...
            For Each sObj In arrArch
                ...
                For ConDet = 0 To xObj.GetElementsByTagName("Detalle").Count - 1
                    ...
                    If oXml.OuterXml.Contains("DscItem") = True Then
                        Try
                            ...
                            If det(ConDet).Dscitem.Contains("HES") Or det(ConDet).Dscitem.Contains("HEM") Then
                                ...
                                For cHes = 0 To heshem.Length - 1
                                    If heshem(cHes).Length = 10 Then
                                        If heshem(cHes).Substring(0, 3) = "100" Or heshem(cHes).Substring(0, 3) = "500" Then
                                            ValHesHem = heshem(cHes)
                                        End If
                                    End If
                ...
                Try
                    If oXml.OuterXml.Contains("<-ºdor>") = True Then
                ...
                Try
                    If (WsBCI_Carga.Equals("S")) Then
                        ...
                        'validacion
                        If WsBCI_krga.Rutrecep = "97006000-6" Then
                            ...
                            Dim Acteco As String = xmlDoc.GetElementsByTagName("Acteco")(0).InnerText
                            ...
                            sqlCmd.CommandText = "select COUNT(*) from [FACELEC_BCI_SAP_BD].[dbo].[dte_recibidos_EMAIL] where Folio='" + folio + "' and TipoDTE='" + Tipodte + "' and RUTEmisor='" + Rutemisor + "'"
                            Dim rcntG As Object = sqlCmd.ExecuteScalar()
                            Dim cc As String = rcntG.ToString()
                            If (cc = 0) Then

                                sql = "INSERT INTO [FACELEC_BCI_SAP_BD].[dbo].[dte_recibidos_EMAIL]([id],[TipoDTE],[Folio],[FchEmis],[RUTEmisor],[RznSoc],[GiroEmis],[Acteco],[CdgSIISucur],[DirOrigen],[CmnaOrigen],[CiudadOrigen],[MntNeto],[MntExe],[TasaIVA],[IVA],[MntTotal],[archivo],[fecRegistro],[estadoAprobComer],[glosaAprobComer],[fechaAprobComer],[fecRecepcion],[estadoRecepcion],[glosaRecepcion])"
                                sql &= "VALUES('" & conta & "','" & Tipodte & "','" & folio & "','" & Fchemis & "','" & Rutemisor & "','" & Rznsoc & "','" & Giroemis & "','" & Acteco & "','" & CdgSIISucur & "','" & Dirorigen & "','" & Cmnaorigen & "','" & Ciudadorigen & "','" & Mntneto & "','" & Mntexe & "','" & Tasaiva & "','" & Iva & "','" & Mnttotal & "','" & xmlDoc.OuterXml.Replace("'", "''") & "'," & "GETDATE()" & ",'" & estadoAprobComer & "','" & glosaAprobComer & "','" & fechaAprobComer & "','" & fecRecepcion & "','" & estadoRecepcion & "','" & glosaRecepcion & "')"
                                sqlCmd.CommandText = sql
                                sqlCmd.ExecuteNonQuery()

                            End If

                            sql = "INSERT INTO [FACELEC_BCI_SAP_BD].[dbo].[dte_recibidos_EMAIL_HISTORIAL]([id],[TipoDTE],[Folio],[FchEmis],[RUTEmisor],[RznSoc],[GiroEmis],[Acteco],[CdgSIISucur],[DirOrigen],[CmnaOrigen],[CiudadOrigen],[MntNeto],[MntExe],[TasaIVA],[IVA],[MntTotal],[archivo],[fecRegistro],[estadoAprobComer],[glosaAprobComer],[fechaAprobComer],[fecRecepcion],[estadoRecepcion],[glosaRecepcion])"
                            sql &= "VALUES('" & conta & "','" & Tipodte & "','" & folio & "','" & Fchemis & "','" & Rutemisor & "','" & Rznsoc & "','" & Giroemis & "','" & Acteco & "','" & CdgSIISucur & "','" & Dirorigen & "','" & Cmnaorigen & "','" & Ciudadorigen & "','" & Mntneto & "','" & Mntexe & "','" & Tasaiva & "','" & Iva & "','" & Mnttotal & "','" & xmlDoc.OuterXml.Replace("'", "''") & "'," & "GETDATE()" & ",'" & estadoAprobComer & "','" & glosaAprobComer & "','" & fechaAprobComer & "','" & fecRecepcion & "','" & estadoRecepcion & "','" & glosaRecepcion & "')"
                            sqlCmd.CommandText = sql
                            sqlCmd.ExecuteNonQuery()
                            sqlCnx.Close()
